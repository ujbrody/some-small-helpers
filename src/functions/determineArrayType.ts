...
function getDetailedType(value: any, options?: DetermineArrayTypeOptions, context: RecursionContext = { depth: 0 }): string {
  ...
  if (typeof value === 'object') {
    ...
    if (Array.isArray(value)) {
      ...
      const innerType = determineArrayType(value, options);
    ...
    const shouldBeVerbose = verboseObjects === 'all' || (verboseObjects === 'top-level' && context.depth === 0);
    
    if (shouldBeVerbose) {
      // Create a new context with increased depth for recursive calls
      const newContext = { depth: context.depth + 1 };
      ...
      const properties = keys.map(key => {
        const propType = getDetailedType(value[key], options, newContext);
        return `${key}: ${propType}`;
      });
...
export default function determineArrayType(arr: any, options?: DetermineArrayTypeOptions) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (Array.isArray(arr[i])) {
      ...
      const arrayType = getDetailedType(arr[i], options);
  ...
  let allObjects = true;
  for (let i = 0; i < arr.length; i++) {
    const isRegularObject = typeof arr[i] === 'object' && 
                           arr[i] !== null && 
                           !Array.isArray(arr[i]) &&
                           (!arr[i].constructor || arr[i].constructor.name === 'Object');
